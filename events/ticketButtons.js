...
client.on("interactionCreate", async (interaction) => {
	if (interaction.isButton()) {
		...
		if (buttonID === "close-ticket-opn") {
         ...
         const role = interaction.guild.roles.cache.find(role => role.name === "Automa Moderatore");
      if (!interaction.member.roles.cache.has(role.id)) {
    return interaction.reply({ content: "Devi aver il ruolo Automa Moderatore.", ephemeral: true });
} 
			await interaction.deferUpdate();
			const userData = await dataTicket.findOne({
				guildID: interaction.guild.id,
